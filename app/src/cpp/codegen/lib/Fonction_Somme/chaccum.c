             ...
             float gradientImg[374400])
{
  ...
  if (flat) {
    ...
    for (b_i = 0; b_i < i; b_i++) {
      b_jj = b_i * 3558;
      ii = (int)fmin(((double)(b_jj + 1) + 3558.0) - 1.0, jj->size[0]);
      if (ii < b_jj + 1) {
        y->size[1] = 0;
      } else {
      ...
      for (b_jj = 0; b_jj < 281; b_jj++) {
        for (idx = 0; idx < i1; idx++) {
          ...
          f1 = roundf((float)Ex_chunk_data[idx] +
                      Gmax * (b_varargin_1[idxE_chunk_data[idx] - 1] / f));
          xc_data[idx + xc->size[0] * b_jj] = f1;
          f = roundf((float)Ey_chunk_data[idx] +
